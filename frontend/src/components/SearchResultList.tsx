...
export function SearchResultList({
  ...
  return (
    <div className="-mx-4">
      {results.map((result) => {
        ...
        return (
          <button
            ...
            className={`grid min-h-16 w-full grid-cols-[4px_minmax(0,1fr)] border-b border-[#DED8CE] text-left transition-colors hover:bg-[#FFFDFC] focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[#B7462D] ${selected ? 'bg-[#B7462D]/10' : ''}`}
          >
            <span className={selected ? 'bg-[#B7462D]' : 'bg-transparent'} />
            <span className="px-4 py-3">
              <span className="block break-words font-medium text-[#24313A]">{result.display_name}</span>
              <span className="mt-1 block break-words text-sm text-[#6B7378]">{result.formatted_address}</span>
      ...
      {nextPageToken && (
        <button onClick={onLoadNext} disabled={isLoadingNext} className="min-h-11 w-full border-b border-[#DED8CE] px-4 py-3 text-sm text-[#35647C] hover:bg-[#FFFDFC] disabled:opacity-50">
