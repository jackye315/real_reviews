    ...
    <div className="grid gap-4">
      {reviews.map((review) => (
        <article key={review.id} className="rounded-2xl border border-[#DED8CE] bg-[#FFFDFC] p-4">
          <div className="flex flex-wrap items-center gap-3 break-words text-sm text-[#6B7378]">
          ...
          <p className="mt-3 whitespace-pre-wrap break-words leading-7 text-[#24313A]">{review.text || review.original_text || 'No review text available.'}</p>
          {review.canonical_source_url && <a className="mt-3 inline-block break-all text-sm text-[#35647C] underline" href={review.canonical_source_url} target="_blank" rel="noreferrer">Original review</a>}
        </article>
      ))}
    </div>
  )
